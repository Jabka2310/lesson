...
install_cursor_free_vip() {
    ...
    if [ -f "${binary_path}" ]; then
        ...
        if [ "$EUID" -ne 0 ]; then
            ...
            if command -v sudo >/dev/null 2>&1; then
                ...
                # Pass custom language cache dir as argument if supported
                sudo "${binary_path}" --language-cache-dir "/home/bronwynm/lesson/s21_string_test/.cursor-free-vip/language_cache"
            else
                echo -e "${YELLOW}⚠️ sudo not found, trying to run normally...${NC}"
                chmod +x "${binary_path}"
                "${binary_path}" --language-cache-dir "/home/bronwynm/lesson/s21_string_test/.cursor-free-vip/language_cache"
            fi
        else
    ...
    if chmod +x "${binary_path}"; then
        ...
        # Run program with custom language cache dir
        if [ "$EUID" -eq 0 ]; then
            "${binary_path}" --language-cache-dir "/home/bronwynm/lesson/s21_string_test/.cursor-free-vip/language_cache"
        else
            sudo "${binary_path}" --language-cache-dir "/home/bronwynm/lesson/s21_string_test/.cursor-free-vip/language_cache"
